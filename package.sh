...
#cp -Rf $CURRENT
cd $TEMP_PATH
sudo mv "Katalon-Studio_Linux_64.tar.gz" "Katalon-Studio_Linux_64-$1-$TIMESTAMP.tar.gz"
sudo mv "Katalon-Studio_MacOS.tar.gz" "Katalon-Studio_MacOS-$1-$TIMESTAMP.tar.gz"
sudo mv "Katalon-Studio_Windows_32.zip" "Katalon-Studio_Windows_32-$1-$TIMESTAMP.zip"
sudo mv "Katalon-Studio_Windows_64.zip" "Katalon-Studio_Windows_64-$1-$TIMESTAMP.zip"
sudo mv "Katalon-Studio.dmg" "Katalon-Studio-MacOS-$1-TIMESTAMP.dmg"
# echo $WORKSPACE
mkdir -p $KATABUILD
if [ ! -d "${KATABUILD}" ]; then
...
cp -Rf "Katalon-Studio_Linux_64-$1-$TIMESTAMP.tar.gz" $KATABUILD/
cp -Rf "Katalon-Studio_Windows_32-$1-$TIMESTAMP.zip" $KATABUILD/
cp -Rf "Katalon-Studio_Windows_64-$1-$TIMESTAMP.zip" $KATABUILD/
cp -Rf "Katalon-Studio-MacOS-$1-$TIMESTAMP.dmg" $KATABUILD/
