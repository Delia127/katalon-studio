...
public class QTestIntegrationTestCaseView extends AbstractTestCaseIntegrationView {
		...
		try {
			if (MessageDialog.openConfirm(null, "Confirmation",
				...
				testCaseEntity.getIntegratedEntities().remove(
						testCaseEntity.getIntegratedEntity(QTestConstants.PRODUCT_NAME));
				reloadView();
				setDirty(true);
			}
		} catch (Exception e) {
	...
	private void performUploadTestCase() {			
		...
		String token = QTestSettingStore.getToken(project.getFolderLocation());
	...
	private void reloadView() {
		IntegratedEntity integratedEntity = testCaseEntity.getIntegratedEntity(QTestConstants.PRODUCT_NAME);
