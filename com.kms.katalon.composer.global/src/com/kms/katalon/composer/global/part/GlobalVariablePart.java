...
import com.kms.katalon.composer.global.support.GlobalVariableNameEdittingSupport;
import com.kms.katalon.composer.global.support.GlobalVariableValueEdittingSupport;
...
public class GlobalVariablePart implements EventHandler {
	...
	private void createComposite(Composite parent) {
		...
		tableViewerColumnName.setEditingSupport(new GlobalVariableNameEdittingSupport(tableViewer, mpart));
		...
		tableViewerColumnValue.setEditingSupport(new GlobalVariableValueEdittingSupport(tableViewer, mpart));
	...
	private void registerControlModifyListeners() {
		...
		tltmEdit.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				Point pt = composite.toDisplay(1, 1);
				GlobalVariableBuilderDialog dialog = new GlobalVariableBuilderDialog(composite.getShell(), selectedVariable, pt);
				...
				if (dialog.open() == Dialog.OK) {
					...
					if (!variableName.equals(variable.getName())) {						
						try {
							setDirty(true);
							save(mpart);
							GroovyRefreshUtil.updateScriptReferencesInTestCaseAndCustomScripts(
									"GlobalVariable." + variableName, 
									"GlobalVariable." + variable.getName(), 
		...
		tltmRefresh.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					if (project != null) {
						if (mpart.isDirty()) {
							MessageDialog.openInformation(tltmRefresh.getDisplay().getActiveShell(), StringConstants.PA_INFO_TITLE,
