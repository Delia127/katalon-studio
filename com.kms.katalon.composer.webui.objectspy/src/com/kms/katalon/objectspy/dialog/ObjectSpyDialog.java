...
public class ObjectSpyDialog extends Dialog implements EventHandler {
			...
			Job job = new Job(StringConstants.DIA_FINDING_ELEMENTS) {
				...
				protected IStatus run(final IProgressMonitor monitor) {
					try {
						...
						monitor.beginTask(MessageFormat.format(StringConstants.DIA_FINDING_ELEMENTS_W_XPATH, xpath), nodeList.getLength());
						...
						Display.getDefault().syncExec(new Runnable() {
							@Override
							public void run() {
								MultiStatusErrorDialog.showErrorDialog(exception, StringConstants.DIA_ERROR_MSG_CANNOT_PARSE_XPATH,
										MessageFormat.format(StringConstants.DIA_ERROR_REASON_INVALID_XPATH, exception.getCause().getMessage()));
							}
						});
	...
	private void addBrowserTypeComboBox(Composite parent) {
		...
		browserTypeCombo.setItems(new String[] { WebUIDriverType.CHROME_DRIVER.toString(), WebUIDriverType.FIREFOX_DRIVER.toString() });
