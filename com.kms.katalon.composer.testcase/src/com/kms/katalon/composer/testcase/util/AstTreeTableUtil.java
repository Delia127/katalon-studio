...
public class AstTreeTableUtil {
	...
	private static List<AstTreeTableNode> getChildren(TryCatchStatement tryCatchStatement, AstTreeTableNode parentNode,
		...
		List<AstTreeTableNode> astTreeTableNodes = new ArrayList<AstTreeTableNode>();
		astTreeTableNodes.add(new AstTryStatementTreeTableNode(tryCatchStatement, parentNode, parentObject, scriptClass));
		for (CatchStatement catchStatement : tryCatchStatement.getCatchStatements()) {
			astTreeTableNodes.add(new AstCatchStatementTreeTableNode(catchStatement, parentNode, tryCatchStatement, scriptClass));
		}
		return astTreeTableNodes;
	}

	private static List<AstTreeTableNode> parseAstObjectIntoTreeTableNode(IfStatement ifStatement,
			AstTreeTableNode parentNode, boolean isElseIf, ASTNode parentObject, IfStatement rootIfStatement, ClassNode scriptClass) {
		List<AstTreeTableNode> astTreeTableNodes = new ArrayList<AstTreeTableNode>();
		if (!isElseIf) {
	...
	private static List<AstTreeTableNode> parseAstObjectIntoTreeTableNode(MethodNode methodNode, ClassNode classNode,
		...
		if (methodNode.getCode() instanceof BlockStatement) {
			...
			if (methodNode.getName().equals(GROOVY_SCRIPT_RUN_METHOD_NAME) && classNode.isScript()) {
				astTreeTableNodes.add(new AstScriptMainBlockStatmentTreeTableNode(blockStatement, null, classNode, classNode));
			} else {
	...
	public static List<AstTreeTableNode> parseAstObjectIntoTreeTableNode(Statement statement,
		...
		if (statement instanceof ExpressionStatement) {
			...
			astTreeTableNodes.add(parseAstObjectIntoTreeTableNode((ForStatement) statement, parentNode, parentObject, scriptClass));
		} else if (statement instanceof WhileStatement) {
			astTreeTableNodes
					.add(parseAstObjectIntoTreeTableNode((WhileStatement) statement, parentNode, parentObject, scriptClass));
		} else if (statement instanceof SwitchStatement) {
			astTreeTableNodes
					.add(parseAstObjectIntoTreeTableNode((SwitchStatement) statement, parentNode, parentObject, scriptClass));
		} else if (statement instanceof CaseStatement) {
			astTreeTableNodes.add(parseAstObjectIntoTreeTableNode((CaseStatement) statement, parentNode, parentObject, scriptClass));
		} else if (statement instanceof AssertStatement) {
			astTreeTableNodes
					.add(parseAstObjectIntoTreeTableNode((AssertStatement) statement, parentNode, parentObject, scriptClass));
	...
	private static AstTreeTableNode parseAstObjectIntoTreeTableNode(CaseStatement caseStatement,
		...
		if (parentObject instanceof SwitchStatement) {
			return new AstCaseStatementTreeTableNode(caseStatement, parentNode, (SwitchStatement) parentObject, scriptClass);
		}
		return null;
	}
		...
		if (expressionStatement.getExpression() instanceof MethodCallExpression) {
			...
			if (isBuiltInKeywordMethodCall((MethodCallExpression) expressionStatement.getExpression())) {
				...
				if (methodCallExpression.getMethodAsString()
					...
					keywordNode = new AstCallTestCaseKeywordTreeTableNode(expressionStatement, parentNode, parentObject, scriptClass);
				} else {
					keywordNode = new AstBuiltInKeywordTreeTableNode(expressionStatement, parentNode, parentObject, scriptClass);
				}
			} else if (isCustomKeywordMethodCall((MethodCallExpression) expressionStatement.getExpression())) {
				keywordNode = new AstCustomKeywordTreeTableNode(expressionStatement, parentNode, parentObject, scriptClass);
			}
			if (keywordNode != null) {
