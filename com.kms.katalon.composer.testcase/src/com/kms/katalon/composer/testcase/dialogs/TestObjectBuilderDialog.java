...
public class TestObjectBuilderDialog extends TreeEntitySelectionDialog implements AstBuilderDialog {
	private final InputValueType[] inputValueTypes = { InputValueType.Variable };
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (haveOtherTypes) {
			...
			List<InputValueType> inputValueTypeForTable = new ArrayList<InputValueType>();
			for (InputValueType inputValueType : inputValueTypes) {
				if (inputValueType != InputValueType.TestObject) {
					inputValueTypeForTable.add(inputValueType);
				}
			}
			tableViewerColumnValueType.setEditingSupport(new AstInputBuilderValueTypeColumnSupport(tableViewer,
					inputValueTypeForTable.toArray(new InputValueType[inputValueTypeForTable.size()]), this,
					scriptClass));
				...
				public void widgetSelected(SelectionEvent e) {
					...
					if (index >= 0 && index < TEST_OBJECT_TABS.length) {
						...
						if (tabName.equals(OBJECT_FINDER_TAB_NAME)) {
							objectExpression = AstTreeTableInputUtil.generateNewExpression(null,
									InputValueType.TestObject);
							stackLayout.topControl = objectFinderComposite;
						} else {
							objectExpression = AstTreeTableInputUtil.generateNewExpression(null, inputValueTypes[0]);
							stackLayout.topControl = inputTableComposite;
