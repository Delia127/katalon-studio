...
public class MobileDeviceDialog extends Dialog implements EventHandler {
	...
	protected Control createDialogArea(Composite parent) {
        ...
        scrImage = new JScrollPane(contextPanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
    ...
    public void highlight(final float x, final float y, final float width, final float height) {
		...
		Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                label.setBounds(Math.round(currentX), Math.round(currentY), Math.round(currentWidth), Math.round(currentHeight));
                // flash
                for (int i = 0; i < 6; i++) {
                    c.add(label);
                    c.revalidate();
	...
	private Image scaleImage(Image image, float newWidth, float newHeight) {
		...
		gc.drawImage(image, 0, 0, image.getBounds().width, image.getBounds().height, 0, 0, Math.round(newWidth), Math.round(newHeight));
		gc.dispose();
		image.dispose();	    
	    return scaled;
	}
	...
	public void handleEvent(Event event) {
		if (event.getTopic().equals(EventConstants.OBJECT_SPY_CLOSE_MOBILE_APP)){
			...
			try{
				...
				if(root != null && root.getAttributes().containsKey("width") && root.getAttributes().containsKey("height")){
					...
					String scaledImageFilePath = System.getProperty("java.io.tmpdir") + File.separator + UUID.randomUUID() + "_scaled1.png";
				...
				//Save scaled version
				String scaledImageFile = System.getProperty("java.io.tmpdir") + File.separator + UUID.randomUUID() + "_scaled2.png";
