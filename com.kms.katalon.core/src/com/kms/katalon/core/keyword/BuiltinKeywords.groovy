...
public class BuiltinKeywords {
	 ...
	 * @param flowControl
	 * @return true if number1 equals number2; otherwise, false
	 * @throws StepFailedException
	 * @throws StepErrorException
	 */
	@CompileStatic
	@Keyword(keywordObject = StringConstants.KW_CATEGORIZE_NUMBER)
	public static boolean verifyEqual(Object actualNumber, Object expectedNumber, FailureHandling flowControl) throws StepFailedException, StepErrorException {
		return KeywordMain.runKeyword({
			boolean isEqual = KeywordHelper.comparingNumberObject(actualNumber, expectedNumber) == 0;
			if (!isEqual) {
				KeywordMain.stepFailed(
						MessageFormat.format(StringConstants.KW_MSG_NUMS_ARE_NOT_EQUAL, String.valueOf(actualNumber), String.valueOf(expectedNumber)), flowControl, null);
				return false;
			} else {
				logger.logPassed(MessageFormat.format(StringConstants.KW_LOG_PASSED_NUMS_ARE_EQUAL, String.valueOf(actualNumber), String.valueOf(expectedNumber)));
			}
			return isEqual;
		}
		, flowControl, MessageFormat.format(StringConstants.KW_MSG_CANNOT_VERIFY_NUMS_ARE_EQUAL, actualNumber, expectedNumber))
	}



	/**
	 * Verify if two numbers are not equal.
	 *
	 * @param actualNumber
	 * @param expectedNumber
	 * @param flowControl
	 * @return true if number1 does not equal number2; otherwise, false
	 * @throws StepFailedException
	 * @throws StepErrorException
	 */
	@CompileStatic
	@Keyword(keywordObject = StringConstants.KW_CATEGORIZE_NUMBER)
	public static boolean verifyNotEqual(Object actualNumber, Object expectedNumber, FailureHandling flowControl) throws StepFailedException, StepErrorException {
		return KeywordMain.runKeyword({
			boolean isEqual = KeywordHelper.comparingNumberObject(actualNumber, expectedNumber) == 0;
			if (isEqual) {
				KeywordMain.stepFailed(MessageFormat.format(StringConstants.KW_MSG_NUMS_ARE_EQUAL, String.valueOf(actualNumber), String.valueOf(expectedNumber)), flowControl, null);
				return false;
			} else {
				logger.logPassed(MessageFormat.format(StringConstants.KW_LOG_PASSED_NUMS_ARE_NOT_EQUAL, String.valueOf(actualNumber), String.valueOf(expectedNumber)));
			}
			return !isEqual;
		}
		, flowControl, MessageFormat.format(StringConstants.KW_MSG_CANNOT_VERIFY_NUMS_ARE_NOT_EQUAL, actualNumber, expectedNumber))
	}
