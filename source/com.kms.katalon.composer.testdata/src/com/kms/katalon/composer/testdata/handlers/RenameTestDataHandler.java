		...
		try {
			if (testDataTreeEntity.getObject() instanceof DataFileEntity) {
				...
				if (code == Window.OK) {
					...
					String oldIdForDisplay = TestDataController.getInstance().getIdForDisplay(testData);
					try {
						if (renameWizard.getNewNameValue() != null && !renameWizard.getNewNameValue().isEmpty()
							...
							String newIdForDisplay = TestDataController.getInstance().getIdForDisplay(testData);
