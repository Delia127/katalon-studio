...
/**
 * This class copy code from CSafariDriverCommandExecutor class to overcome it's package-protected fields
 ...
 *
 */
public class CSafariDriverCommandExecutor implements CommandExecutor {
    private static final Logger log = Logger.getLogger(CSafariDriverCommandExecutor.class.getName());

    private final CSafariDriverServer server;

    private final BrowserLocator browserLocator;

    private final CSessionData sessionData;

    private final boolean cleanSession;

    private CommandLine commandLine;

    private CWebSocketConnection connection;

    /**
     * @param options The {@link SafariOptions} instance
     */
    public CSafariDriverCommandExecutor(CSafariOptions options) {
      this.server = new CSafariDriverServer(options.getPort());
      this.browserLocator = new SafariLocator();
      this.sessionData = CSessionData.forCurrentPlatform();
    ...
    public CSafariDriverServer getServer() {
        return server;
    }
