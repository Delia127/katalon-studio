...
import static org.codehaus.groovy.eclipse.codeassist.requestor.ContentAssistLocation.ANNOTATION;
...
import com.kms.katalon.composer.codeassist.util.ImprovedGroovyUtils;

/**
 * Finds the completion node for an offset and calculates the content assist context.
 */
public class ImprovedCompletionNodeFinder extends ClassCodeVisitorSupport {
    ...
    public ImprovedCompletionNodeFinder(int completionOffset, int completionEnd, int supportingNodeEnd,
    ...
    public void visitImports(ModuleNode node) {
        ...
        for (ImportNode importNode : ImprovedGroovyUtils.getAllImportNodes(node)) {
            visitAnnotations(importNode);
            if (importNode.getType() == null && StringUtils.isEmpty(completionExpression)
                    || doTest(importNode.getType())) {
    ...
    protected void visitAnnotation(AnnotationNode annotation) {
        ...
        int annoEnd = ImprovedGroovyUtils.endOffset(annotation), nameEnd = annotation.getClassNode().getEnd();
        if (annoEnd > nameEnd) { // annotation has a body
            if (completionOffset > nameEnd && completionOffset < annoEnd) {
                ...
                createContext(annotation, annotation, ContentAssistLocation.ANNOTATION);
            }
        }
        blockStack.removeLast();
    }
    ...
    public void visitMethodCallExpression(MethodCallExpression expression) {
        ...
        createContextForCallContext(expression, methodExpression,
                ...
                // this is not exactly right since it will fail on funky kinds of method calls, like those that are
                // called by a GString
    ...
    private void checkForAfterClosingParen(AnnotatedNode contextTarget, Expression arguments) {
        ...
        if (start == 0 && arguments instanceof TupleExpression
                && !((TupleExpression) arguments).getExpressions().isEmpty()) {
    ...
    private void createContextForCallContext(Expression origExpression, AnnotatedNode methodExpr, String methodName) {
        context = new MethodInfoContentAssistContext(completionOffset, completionExpression, fullCompletionExpression,
                origExpression, blockStack.getLast(), lhsNode, unit, currentDeclaration, completionEnd, methodExpr,
                methodName, methodExpr.getEnd());
        throw new VisitCompleteException();
    }
    ...
    protected boolean doTest(ASTNode node) {
        ...
        return node.getEnd() > 0 && ((supportingNodeEnd > node.getStart() && supportingNodeEnd <= node.getEnd())
