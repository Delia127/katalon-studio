...
/**
 * Clone from {@link CodeSelectRequestor}
 * Bug fixes for {@link #findElement(IType, String, Parameter[])} method
 *
 */
@SuppressWarnings("restriction")
public class CustomCodeSelectRequestor extends CodeSelectRequestor {
        ...
        if (doTest(node)) {
            ...
            if (requestedNode != null) {
                if (result.declaration instanceof VariableExpression) {
                    ...
                    requestedElement = createLocalVariable(result, enclosingElement, var);
                    ...
                    try {
                        requestedElement = createLocalVariable(result, (JavaElement) unit.getElementAt(position), var);
                    ...
                    if (declaringType != null) {
                        ...
                        if (type != null) {
                            try {
                                ...
                                // try to resolve the type of the requested element. this will add the proper type
                                // parameters, etc to the hover
    ...
    private LocalVariable createLocalVariable(TypeLookupResult result, IJavaElement enclosingElement, Variable var) {
        ...
        return ReflectionUtils.createLocalVariable(enclosingElement, var.getName(), start,
                Signature.createTypeSignature(createGenericsAwareName(type, true), false));
    }
    ...
    private String createUniqueKey(AnnotatedNode node, ClassNode resolvedType, ClassNode resolvedDeclaringType,
        ...
        if (node instanceof FieldNode) {
            ...
            if (maybeRequested.getElementType() == IJavaElement.FIELD) {
                ...
                return createUniqueKeyForGeneratedAccessor((MethodNode) node, resolvedType, resolvedDeclaringType,
                        (IField) maybeRequested).toString();
            } else {
    ...
    private StringBuilder createUniqueKeyForMethod(MethodNode node, ClassNode resolvedType,
    ...
    private StringBuilder createUniqueKeyForField(FieldNode node, ClassNode resolvedType,
            ClassNode resolvedDeclaringType) {
        StringBuilder sb = new StringBuilder();
    ...
    private StringBuilder createUniqueKeyForGeneratedAccessor(MethodNode node, ClassNode resolvedType,
            ClassNode resolvedDeclaringType, IField actualField) {
        StringBuilder sb = new StringBuilder();
        sb.append(createUniqueKeyForClass(node.getDeclaringClass(), resolvedDeclaringType));
        sb.append('.').append(actualField.getElementName()).append(')');
        ClassNode typeOfField = node.getName().startsWith("set") && node.getParameters() != null
                && node.getParameters().length > 0 ? node.getParameters()[0].getType() : resolvedType;
    ...
    private StringBuilder createUniqueKeyForResolvedClass(ClassNode resolvedType) {
        ...
        return new StringBuilder(
                Signature.createTypeSignature(createGenericsAwareName(resolvedType, false/* fully qualified */), true/*
                                                                                                                      * must
                                                                                                                      * resolve
                                                                                                                      */)
                        .replace('.', '/'));
    ...
    private IJavaElement findElement(IType type, String text, Parameter[] parameters) throws JavaModelException {
        ...
        for (IMethod method : type.getMethods()) {
            ...
            if (maybeMethodParameters.length == parameters.length
                    && isCorrectMethod(type.isBinary(), parameters, maybeMethodParameters)) {
                return method;
            }
        }
        ...
        for (IMethod method : type.getMethods()) {
            if (method.getElementName().equals(setMethod) || method.getElementName().equals(getMethod)
