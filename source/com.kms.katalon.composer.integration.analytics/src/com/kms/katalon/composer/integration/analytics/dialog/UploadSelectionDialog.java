        ...
        try {
            new ProgressMonitorDialog(getShell()).run(true, true, new IRunnableWithProgress() {
                ...
                public void run(IProgressMonitor monitor) {
                    try {
                        monitor.beginTask(ComposerIntegrationAnalyticsMessageConstants.REPORT_MSG_UPLOADING_TO_ANALYTICS, 2);
                        monitor.subTask(ComposerIntegrationAnalyticsMessageConstants.REPORT_MSG_UPLOADING_TO_ANALYTICS_SENDING);
                        analyticsReportService.upload(testSuiteLogRecord.getLogFolder());
                        monitor.worked(1);
                        monitor.subTask(ComposerIntegrationAnalyticsMessageConstants.REPORT_MSG_UPLOADING_TO_ANALYTICS_SUCCESSFULLY);
                        monitor.worked(2);
                    } catch (AnalyticsApiExeception ex) {
                        LoggerSingleton.logError(ex);
                        MultiStatusErrorDialog.showErrorDialog(ex,
                                ComposerIntegrationAnalyticsMessageConstants.REPORT_ERROR_MSG_UNABLE_TO_UPLOAD_REPORT,
                                ex.getMessage());
                    }
                }
            });
            ...
            LoggerSingleton.logError(ex);
            MultiStatusErrorDialog.showErrorDialog(ex,
                    ComposerIntegrationAnalyticsMessageConstants.REPORT_ERROR_MSG_UNABLE_TO_UPLOAD_REPORT, ex.getMessage());
        }
    }
