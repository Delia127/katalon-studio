...
public class CustomPushResultDialog extends TitleAreaDialog {
    ...
    CustomPushResultDialog(final Shell parentShell, final Repository localDb, final PushOperationResult result,
            final String destinationString, boolean modal) {
        super(parentShell);
     ...
     */
    public static void show(final Repository repository, final PushOperationResult result, final String sourceString,
            final boolean showConfigureButton, final boolean modal) {
        PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
            @Override
            public void run() {
                Shell shell = PlatformUI.getWorkbench().getModalDialogShellProvider().getShell();
                CustomPushResultDialog dialog = new CustomPushResultDialog(shell, repository, result, sourceString,
                        modal);
    ...
    protected void createButtonsForButtonBar(final Composite parent) {
        if (!hideConfigure && SimpleConfigurePushDialog.getConfiguredRemote(localDb) != null) {
            createButton(parent, CONFIGURE, UIText.PushResultDialog_ConfigureButton, false);
        }
    ...
    protected void buttonPressed(int buttonId) {
        ...
        if (buttonId == CONFIGURE) {
            ...
            PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
                @Override
                public void run() {
                    Dialog dlg = SimpleConfigurePushDialog.getDialog(PlatformUI.getWorkbench()
    ...
    protected Control createDialogArea(final Composite parent) {
        ...
        if (pushErrors != null && pushErrors.length() > 0) {
            ...
            title = NLS.bind(UIText.PushResultDialog_label_failed, destinationString);
        } else {
            title = NLS.bind(UIText.PushResultDialog_label, destinationString);
        }
        setTitle(title);
        final CustomPushResultTable table = new CustomPushResultTable(composite, getDialogBoundsSettings());
        table.setData(localDb, result);
        final Control tableControl = table.getControl();
        final GridData tableLayout = new GridData(SWT.FILL, SWT.FILL, true, true);
        tableLayout.widthHint = 650;
        tableLayout.heightHint = 300;
        tableControl.setLayoutData(tableLayout);

        getShell().setText(NLS.bind(UIText.PushResultDialog_title, destinationString));
