...
public class CustomConfirmationPage extends WizardPage {
    ...
    private CustomPushResultTable resultPanel;

    private UserPasswordCredentials credentials;

    private String helpContext = null;

    public CustomConfirmationPage(final Repository local) {
        super(CustomConfirmationPage.class.getName());
        this.local = local;
    ...
    public static Collection<RemoteRefUpdate> copyUpdates(final Collection<RemoteRefUpdate> refUpdates)
            throws IOException {
    ...
    public void createControl(final Composite parent) {
        ...
        resultPanel = new CustomPushResultTable(panel);
    ...
    private void checkPreviousPagesSelections(RepositorySelection repositorySelection, List<RefSpec> refSpecs) {
        if (!repositorySelection.equals(displayedRepoSelection) || !refSpecs.equals(displayedRefSpecs)) {
    ...
    private void revalidateImpl() {
        ...
        try {
            final Collection<RemoteRefUpdate> updates = Transport.findRemoteRefUpdatesFor(local, displayedRefSpecs,
                    fetchSpecs);
            if (updates.isEmpty()) {
        ...
        if (result.isSuccessfulConnectionForAnyURI()) {
            ...
            ErrorDialog.openError(getShell(), UIText.ConfirmationPage_cantConnectToAnyTitle, null, new Status(
                    IStatus.ERROR, Activator.getPluginId(), message));
        }
    }
}
