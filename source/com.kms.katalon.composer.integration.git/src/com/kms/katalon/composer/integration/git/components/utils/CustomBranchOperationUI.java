...
public class CustomBranchOperationUI {
     ...
     *
     * @param repository
     * @return the {@link CustomBranchOperationUI}
     */
    public static CustomBranchOperationUI rename(Repository repository) {
        return new CustomBranchOperationUI(repository, MODE_RENAME);
    }
     ...
     * @return the {@link CustomBranchOperationUI}
     */
    public static CustomBranchOperationUI delete(Repository repository) {
        return new CustomBranchOperationUI(repository, MODE_DELETE);
    }
     ...
     * @return the {@link CustomBranchOperationUI}
     */
    public static CustomBranchOperationUI create(Repository repository) {
        CustomBranchOperationUI op = new CustomBranchOperationUI(repository, MODE_CREATE);
        return op;
    }
     ...
     * @return the {@link CustomBranchOperationUI}
     */
    public static CustomBranchOperationUI createWithRef(Repository repository, String baseRef) {
        CustomBranchOperationUI op = new CustomBranchOperationUI(repository, MODE_CREATE);
        op.base = baseRef;
        return op;
    }
     ...
     * @return the {@link CustomBranchOperationUI}
     */
    public static CustomBranchOperationUI checkout(Repository repository) {
        return new CustomBranchOperationUI(repository, MODE_CHECKOUT);
    }
     ...
     * @return the {@link CustomBranchOperationUI}
     */
    public static CustomBranchOperationUI checkout(Repository repository, String target) {
        return new CustomBranchOperationUI(repository, target, true);
    }
     ...
     * @return the {@link CustomBranchOperationUI}
     */
    public static CustomBranchOperationUI checkoutWithoutQuestion(Repository repository, String target) {
        return new CustomBranchOperationUI(repository, target, false);
    }
     ...
     */
    private CustomBranchOperationUI(Repository repository, String target, boolean showQuestionsBeforeCheckout) {
     ...
     * @param mode
     */
    private CustomBranchOperationUI(Repository repository, int mode) {
        this.repository = repository;
        this.mode = mode;
    ...
    public void start() {
        if (!repository.getRepositoryState().canCheckout()) {
            MessageDialog.openError(getShell(), UIText.BranchAction_cannotCheckout,
                    NLS.bind(UIText.BranchAction_repositoryState, repository.getRepositoryState().getDescription()));
            return;
        }
        ...
        final boolean restore = Activator.getDefault()
                .getPreferenceStore()
                .getBoolean(UIPreferences.CHECKOUT_PROJECT_RESTORE);
        final BranchOperation bop = new BranchOperation(repository, target, !restore);
        ...
        Job job = new WorkspaceJob(jobname) {
            ...
            public IStatus runInWorkspace(IProgressMonitor monitor) {
                try {
                    if (restore) {
                        final CustomBranchProjectTracker tracker = new CustomBranchProjectTracker(repository);
    ...
    public void run(IProgressMonitor monitor) throws CoreException {
        if (!repository.getRepositoryState().canCheckout()) {
            PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
                @Override
                public void run() {
                    MessageDialog.openError(getShell(), UIText.BranchAction_cannotCheckout, NLS.bind(
                            UIText.BranchAction_repositoryState, repository.getRepositoryState().getDescription()));
                }
            });
            return;
        }
    ...
    private void askForTargetIfNecessary() {
        ...
        if (target != null && showQuestionsBeforeCheckout && shouldShowCheckoutRemoteTrackingDialog(target)) {
    ...
    private static boolean shouldShowCheckoutRemoteTrackingDialog(String refName) {
        boolean isRemoteTrackingBranch = refName != null && refName.startsWith(Constants.R_REMOTES);
    ...
    private String getTargetWithDialogInUI() {
        ...
        switch (mode) {
            case MODE_CHECKOUT:
                try {
                    dialog = new CheckoutLocalBranchSourceSelectionDialog(getShell(), repository, repository.getFullBranch());
                } catch (IOException e) {
    ...
    private static class CheckoutLocalBranchSourceSelectionDialog extends AbstractBranchSelectionDialog {
        public CheckoutLocalBranchSourceSelectionDialog(Shell parentShell, Repository repository, String refToMark) {
            super(parentShell, repository, refToMark, SHOW_LOCAL_BRANCHES | SELECT_CURRENT_REF
            ...
            PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
                ...
                public void run() {
                    ...
                    CleanupUncomittedChangesDialog cleanupUncomittedChangesDialog = new CleanupUncomittedChangesDialog(
                            shell, UIText.BranchResultDialog_CheckoutConflictsTitle, NLS.bind(
                                    UIText.BranchResultDialog_CheckoutConflictsMessage,
                                    Repository.shortenRefName(target)), repository, result.getConflictList());
                    cleanupUncomittedChangesDialog.open();
                    if (cleanupUncomittedChangesDialog.shouldContinue()) {
                        CustomBranchOperationUI op = CustomBranchOperationUI.checkoutWithoutQuestion(repository, target);
                        op.start();
                    }
                }
            });
        ...
        PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
            ...
            public void run() {
                ...
                if (store.getBoolean(UIPreferences.SHOW_DETACHED_HEAD_WARNING)) {
                    ...
                    MessageDialogWithToggle.openInformation(PlatformUI.getWorkbench()
                            .getActiveWorkbenchWindow()
                            .getShell(), UIText.BranchOperationUI_DetachedHeadTitle,
    ...
    private boolean shouldCancelBecauseOfRunningLaunches(IProgressMonitor monitor) {
    ...
    private ILaunchConfiguration getRunningLaunchConfiguration(IProgressMonitor monitor) {
        ...
        try {
            ModalContext.run(new IRunnableWithProgress() {
                ...
                public void run(IProgressMonitor m) throws InvocationTargetException, InterruptedException {
                    Set<IProject> projects = new HashSet<>(Arrays.asList(ProjectUtil.getProjects(repository)));
