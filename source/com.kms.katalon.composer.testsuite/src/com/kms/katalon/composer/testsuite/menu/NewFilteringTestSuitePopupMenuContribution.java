    ...
    public void aboutToShow(List<MMenuElement> menuItems) {
        try {
            ...
            if (NewTestSuiteHandler.findParentTreeEntity(selectedObjects) != null
                    && !FilterController.getInstance().getPluginKeywords().isEmpty()) {
                MHandledMenuItem newTestSuitePopupMenuItem = MenuFactory.createPopupMenuItem(
                        commandService.createCommand(NEW_FILTERING_TEST_SUITE_COMMAND, null),
                        "Filtering Test Suite", ConstantsHelper.getApplicationURI());
