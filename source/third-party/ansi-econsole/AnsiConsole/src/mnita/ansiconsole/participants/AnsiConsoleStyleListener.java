...
public class AnsiConsoleStyleListener implements LineStyleListener {
    ...
    public void lineGetStyle(LineStyleEvent event) {
        ...
        if (!ranges.isEmpty())
        	addHarStyle(event, ranges);	
            event.styles = ranges.toArray(new StyleRange[ranges.size()]);
    }
    
    private void addHarStyle(LineStyleEvent event, List<StyleRange> ranges) {
        String lineText = event.lineText;
        String har = "HAR: ";
        String harExtension = ".har";
        if (lineText.contains(har)) {
            int harLogStart = lineText.indexOf(harExtension);
            int harLogEnd = lineText.lastIndexOf(harExtension);
            if (harLogStart > 0 && harLogEnd > 0 && harLogEnd > harLogStart) {
                int harFilePathStart = harLogStart + har.length();
                int harFilePathEnd = harLogEnd + harExtension.length() - 1;
