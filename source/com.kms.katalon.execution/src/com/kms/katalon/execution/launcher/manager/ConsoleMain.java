...
public class ConsoleMain {
			...
			while (offset < arguments.length) {
				if (arguments[offset].startsWith(PROJECT_PK_ARGUMENT)) {
					...
					if (arguments[offset + 1].startsWith(TESTSUITE_ID_ARGUMENT)
						...
						String testSuiteID = arguments[offset + 1].substring(TESTSUITE_ID_ARGUMENT.length());
						String argBrowserType = arguments[offset + 2].substring(BROWSER_TYPE_ARGUMENT.length());
						...
						ExecutionEntity executionEntity = addExecutionEntities(testSuiteID, argBrowserType,
								projectEntity, testSuiteReportFolder);
	...
	private ExecutionEntity addExecutionEntities(String testSuiteID, String argBrowserType,
			ProjectEntity projectEntity, String reportFolderPath) throws InterruptedException, CoreException {
		try {
