...
//        return recordedActions.size();
	...
	private void createLeftPanel(Composite parent) {
		...
		capturedObjectComposite.addListener(objectPropertiesView,
				Arrays.asList(ObjectSpyEvent.SELECTED_ELEMENT_CHANGED));
		// capturedObjectComposite.addListener(this,
		// Arrays.asList(ObjectSpyEvent.SELECTED_ELEMENT_CHANGED));
		...
		this.addListener(verifyView,
				Arrays.asList(ObjectSpyEvent.ADDON_SESSION_STARTED, ObjectSpyEvent.SELENIUM_SESSION_STARTED));
		// objectPropertiesView.addListener(this,
		// Arrays.asList(ObjectSpyEvent.REQUEST_DIALOG_RESIZE));
	}
	...
	private void deleteSelectedItems() {
		...
		List<HTMLActionMapping> selectedActionMappings = Arrays.asList(selection.toArray()).stream()
				.filter(selectedObject -> selectedObject instanceof HTMLActionMapping)
				.map(selectedObject -> (HTMLActionMapping) selectedObject).collect(Collectors.toList());
	...
	// recordedActions.size() > 0 ? recordedActions.get(recordedActions.size() -
	// 1) : null);
	// }
	...
	private boolean isEquals(WebElement elm1, WebElement elm2) {
		return new EqualsBuilder().append(elm1.getType(), elm2.getType()).append(elm1.getTag(), elm2.getTag())
				.append(elm1.hasProperty(), elm2.hasProperty()).append(elm1.getXpath(), elm2.getXpath()).isEquals();
	}
