...
public abstract class SimpleWizardDialog extends Dialog implements IWizardPageChangedListerner {
    ...
    public static final int FINISH_BUTTON_ID = 3;

    public static final int SKIP_BUTTON_ID = 0;
    ...
    protected Composite createButtonBarCompositeSkip(Composite parent) {
        Composite buttonBarComposite = new Composite(parent, SWT.NONE);
        GridLayout layout = new GridLayout();
        buttonBarComposite.setLayout(layout);
    ...
    private void updateButtonBar(IWizardPage page) {
        getButton(FINISH_BUTTON_ID).setEnabled(page.canFinish() && wizardManager.getWizardPages().indexOf(page) == 7);
