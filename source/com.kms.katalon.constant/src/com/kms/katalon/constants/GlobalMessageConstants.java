package com.kms.katalon.constants;

import org.eclipse.osgi.util.NLS;

public class GlobalMessageConstants extends NLS {
    private static final String BUNDLE_NAME = "com.kms.katalon.constants.globalMessages";

	public static String DIA_OPEN_PROJECT;

    public static String APP_NAME;

    public static String ADD;

    public static String INSERT;

    public static String DELETE;

    public static String REMOVE;

    public static String CLEAR;

    public static String EDIT;

    public static String CUT;

    public static String COPY;

    public static String PASTE;

    public static String REFRESH;

    public static String SEARCH;

    public static String FORMAT;

    public static String SELECT_ALL;

    public static String UNDO;

    public static String REDO;

    public static String BROWSE;

    public static String FIND;

    public static String UP;

    public static String DOWN;

    public static String UPDATE;

    public static String EMPTY;

    public static String PROPERTIES;

    public static String CONFIRMATION;

    public static String NOTIFICATION;

    public static String PROJECT_SETTINGS;

    public static String ALL;

    public static String PASSED;

    public static String FAILED;

    public static String INFO;

    public static String WARN;

    public static String ERROR;

    public static String INCOMPLETE;

    public static String NOT_RUN;

    public static String COMPLETE;

    public static String NOT_STARTED;

    public static String FOLDER;

    public static String GIT;

    public static String JAR;

    public static String PACKAGE;

    public static String DEFAULT_PACKAGE_NAME;

    public static String TEST_CASE;

    public static String TEST_SUITE;
    
    public static String TEST_SUITE_COLLECTION;

    public static String TEST_OBJECT;

    public static String TEST_DATA;

    public static String KEYWORD;

    public static String REPORT;

    public static String CHECKPOINT;
    
    public static String TEST_LISTENER;

    public static String OBJECT_SPY;

    public static String WEB_RECORDER;

    public static String RUN;

    public static String STOP;

    public static String ID;

    public static String NAME;

    public static String TYPE;

    public static String VALUE;

    public static String IMAGE;

    public static String DESCRIPTION;

    public static String SUMMARY;

    public static String INFORMATION;

    public static String COMMENT;

    public static String TAG;

    public static String INPUT;

    public static String OUTPUT;

    public static String OBJECT;

    public static String EMAIL;

    public static String LEVEL;

    public static String TIME;

    public static String MESSAGE;

    public static String NOT_FOUND;

    public static String DONE;

    public static String NO_;

    public static String ATTACHMENT;

    public static String STATUS;

    public static String ANDROID;

    public static String IOS;

    public static String ROOT_FOLDER_NAME_TEST_CASE;

    public static String ROOT_FOLDER_NAME_TEST_SUITE;

    public static String ROOT_FOLDER_NAME_DATA_FILE;

    public static String ROOT_FOLDER_NAME_OBJECT_REPOSITORY;

    public static String ROOT_FOLDER_NAME_KEYWORD;

    public static String ROOT_FOLDER_NAME_REPORT;

    public static String ROOT_FOLDER_NAME_CHECKPOINT;

    public static String ROOT_FOLDER_NAME_PROFILES;

    public static String ROOT_FOLDER_NAME_TEST_LISTENER;

    public static String ROOT_FOLDER_NAME_FEATURES;
    
    public static String ROOT_FOLDER_NAME_CONFIG;
    
    public static String ROOT_FOLDER_NAME_INCLUDE;
    
    public static String SYSTEM_FOLDER_NAME_BIN;
    
    public static String SYSTEM_FOLDER_NAME_DRIVER;
            
    public static String SYSTEM_FOLDER_NAME_LIB;
            
    public static String SYSTEM_FOLDER_NAME_PLUGIN;
    
    public static String SYSTEM_FOLDER_NAME_SETTINGS;
    
    public static String SYSTEM_FOLDER_NAME_SCRIPT;

    public static String FILE_NAME_GLOBAL_VARIABLE;

    public static String NULL;

    public static String UNKNOWN;

    public static String OK;

    public static String TEST_CASE_SCRIPT_ROOT_FOLDER_NAME;

    public static String ROOT_FOLDER_NAME_DRIVERS;

    public static String WZ_SETUP_BTN_BACK;

    public static String WZ_SETUP_BTN_NEXT;

    public static String DIA_FINISH;
    
    public static String DIA_OK;

    public static String DIA_CANCEL;

    public static String DIA_CLOSE;

    public static String DIA_YES;

    public static String DIA_NO;

    public static String CREATED_DATE;

    public static String MODIFIED_DATE;

    public static String RECENT;
    
    public static String OBJECT_FOLDER;

    public static String ELAPSED;

    public static String SIZE;
    
    public static String APPIUM_INSTALLATION_GUIDE_MSG;

    public static String URL_TROUBLESHOOTING_MOBILE_TESTING;
    
    public static String DIA_LET_START;
    
    public static String DIA_NEW_PROJECT;
    
    public static String DIA_NAME_EXISTED;

    public static String DIA_TITLE_QUICKSTART;
    
    static {
        // initialize resource bundle
        NLS.initializeMessages(BUNDLE_NAME, GlobalMessageConstants.class);
    }

    private GlobalMessageConstants() {
    }
}
