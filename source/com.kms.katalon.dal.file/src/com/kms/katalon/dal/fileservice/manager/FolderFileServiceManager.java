...
public class FolderFileServiceManager {
	...
	/**
	 * Use to create new Folders (Test Case folder, Test Suite folder, Test Data
	 * folder...)
	 * 
	 * @param parentFolder
	...
	public static FolderEntity addNewFolder(FolderEntity parentFolder, String defaultName) throws Exception {
		if (parentFolder != null) {
			...
			if (newFolder.getFolderType() == FolderType.TESTCASE) {
				...
				GroovyRefreshUtil.refreshFolder(GroovyUtil.getRelativePathForFolder(parentFolder), parentFolder.getProject(), null);
			}
				...
				for (File file : parentFolderFile.listFiles(EntityFileServiceManager.fileFilter)) {
					if (file.isDirectory()) {
						if (FileUtils.directoryContains(file, new File(file, "execution0.log"))) {
							ReportEntity report = ReportFileServiceManager.createReportEntity(
									FilenameUtils.getBaseName(file.getName()), parentFolderEntity);
							list.add(report);
							sortByDate = true;
						}
						else {
					...
					if (updated) {
						...
						case TESTCASE:							
							...
							if (oldScriptFolder.exists()) {
								...
								GroovyUtil.getTestCaseScriptSourceFolder(project).refreshLocal(IResource.DEPTH_INFINITE, null);
							}
