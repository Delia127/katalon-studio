...
public class BuiltInMethodNodeFactory {
    ...
    public IKeywordContributor[] getKeywordContributors() {
        List<IKeywordContributor> keywordContributors = KeywordContributorCollection.getInstance()
        ...
        return keywordContributors.toArray(new IKeywordContributor[keywordContributors.size()]);
    }
    ...
    public List<String> getParameterNames(Method method) throws IOException {
        ...
        for (MethodNode node : (List<MethodNode>) classNode.methods) {
            if (node.name.equals(method.getName())) {
                List<String> parameterNames = new ArrayList<String>(node.localVariables.size());
