...
public class LogExceptionNavigator {	
	...
	public MPart getTestCaseGroovyEditor(TestCaseEntity testCase) {
		...
		MPartStack stack = (MPartStack) modelService.find(IdConstants.COMPOSER_CONTENT_PARTSTACK_ID,
			application);
		if (stack != null) {
			String testCaseCompositePartId = EntityPartUtil
					.getTestCaseCompositePartId(testCase.getId());
			MCompositePart mcompositePart = (MCompositePart) modelService.find(testCaseCompositePartId,
					stack);
			TestCaseCompositePart testCaseCompositePart = (TestCaseCompositePart) mcompositePart
					.getObject();
	...
	public void openKeywordByLogException(XmlLogRecordException logException) {
		try {
	    	...
	    	if (classIndex > 0) {
	        	...
	        	customKeywordScriptFile = keywordPackageFolder.getFile(className + GroovyConstants.GROOVY_FILE_EXTENSION);
	    	} else {
