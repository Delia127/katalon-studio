...
public class RestRequestObjectPart extends RequestObjectPart{
	...
	protected void createServiceInfoComposite(Composite mainComposite) {
		...
		ExpandableComposite restComposite = new ExpandableComposite(mainComposite, StringConstants.PA_TITLE_REST, 1, true);
	...
	protected void updateEntityBeforeSaved(){
		...
		originalObject.setRestUrl(txtRestUrl.getText());
		originalObject.setRestRequestMethod(cbbRestRequestMethod.getText());
		originalObject.setRestParameters(listRestParams);
	...
	protected void showEntityFieldsToUi(){
		...
		txtRestUrl.setText(originalObject.getRestUrl());
		int index = Arrays.asList(WebServiceRequestEntity.REST_REQUEST_METHODS).indexOf(originalObject.getRestRequestMethod());
		cbbRestRequestMethod.select(index < 0 ? 0 : index);
		listRestParams.addAll(originalObject.getRestParameters());		
