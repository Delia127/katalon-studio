...
public class DataFileFileServiceManager {
	...
	public static DataFileEntity addNewDataFile(FolderEntity parentFolder) throws Exception {
		if (parentFolder != null) {
			...
			String name = EntityService.getInstance().getAvailableName(parentFolder.getLocation(), StringConstants.MNG_NEW_TEST_DATA,
					true);
	 ...
	 * @throws Exception
	 */
	private static String isDataFileReferenced(DataFileEntity dataFile, List<TestCaseEntity> testCaseList,
			List<TestSuiteEntity> testSuiteList) throws Exception {
		if (dataFile != null) {
			String referenceName = "";

			for (TestCaseEntity testCaseEntity : testCaseList) {
				if (testCaseEntity.getDataFileLocations().contains(dataFile.getRelativePath())) {
					referenceName += " " + testCaseEntity.getName() + ";";
				}
			}
	...
	public static void deleteDataFile(DataFileEntity dataFile) throws Exception {
		if (dataFile != null) {
			String referenceString = isDataFileReferenced(dataFile,
					FolderFileServiceManager.getDescendantTestCasesOfFolder(FolderFileServiceManager
							.getTestCaseRoot(dataFile.getProject())),
					FolderFileServiceManager.getDescendantTestSuitesOfFolder(FolderFileServiceManager
							.getTestSuiteRoot(dataFile.getProject())));
	...
	/**
	 * check if a folder entity that have data file entities can be deleted or
	 * not
	 * 
	 * @param folder
	 * @param projectPk
	 ...
	 * @throws Exception
	 */
	private static String canDeleteDataFileFolder(FolderEntity folder, List<TestCaseEntity> testCaseList,
			List<TestSuiteEntity> testSuiteList) throws Exception {
		String referenceString = "";
		for (FileEntity childEntity : FolderFileServiceManager.getChildren(folder)) {
			if (childEntity instanceof FolderEntity) {
				referenceString += canDeleteDataFileFolder((FolderEntity) childEntity, testCaseList, testSuiteList);
			} else if (childEntity instanceof DataFileEntity) {
				referenceString += isDataFileReferenced((DataFileEntity) childEntity, testCaseList, testSuiteList);
			}
		}
	...
	public static void deteleDataFileFolder(FolderEntity folder) throws Exception {
		if (folder != null) {
			String referenceString = canDeleteDataFileFolder(folder,
					FolderFileServiceManager.getDescendantTestCasesOfFolder(FolderFileServiceManager
							.getTestCaseRoot(folder.getProject())),
					FolderFileServiceManager.getDescendantTestSuitesOfFolder(FolderFileServiceManager
		...
		if (!testDataEntity.getName().equals(dataFilePropertyInput.getName())) {
			if (!testDataEntity.getName().equalsIgnoreCase(dataFilePropertyInput.getName())) {
				...
				if (!availableName.equalsIgnoreCase(dataFilePropertyInput.getName())) {
					...
					(MessageFormat.format(StringConstants.MNG_EXC_EXISTED_DATA_FILE_NAME, dataFilePropertyInput.getName()));
				}
			}
	...
	/**
	 * update all test cases, test suites and job definition that each one refers to the given
	 * data file after it renamed
	 * 
	 * @param oldEntity
	 * @param newEntity
	 * @param project
	...
	private static void updateReferencesAfterDataFileRenamed(String oldTestDataLocation, DataFileEntity newEntity,
		...
		if (!oldTestDataLocation.equals(newEntity.getLocation())) {
			...
			//TODO need to update reference between test suite and the given data file 
