...
public class ObjectPropertyView {
	...
	private void createTestObjectInfoComposite() {
		...
		lblNewLabel.setText(StringConstants.VIEW_LBL_INFO);
		...
		imageUtilComp.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, true, 2, 1));
		imageUtilComp.setLayout(new GridLayout(2, false));
		...
		btnBrowseImage.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (radRelative.getSelection()) {
					String relPath = new File(projectFolder).toURI().relativize(new File(absolutePath).toURI())
							.getPath();
		...
		lblDescription.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, true, 1, 1));
	...
	private void createTableDetails() {
		...
		trclmnColumnSelected.setWidth(100);
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					if (txtImage.getText() != null && !txtImage.getText().trim().equals("")) {
						...
						if (radRelative.getSelection()) {
							String relPath = new File(projectFolder).toURI().relativize(new File(thePath).toURI())
									.getPath();
							txtImage.setText(relPath);
						} else {
							txtImage.setText((projectFolder + File.separator + thePath).replace("/", "\\"));
						}
					}
					dirtyable.setDirty(true);
				} catch (Exception e1) {
