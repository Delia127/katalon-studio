...
public class PreferencesRegistry {
	...
	public PreferenceManager getPreferenceManager(String perferencePage) {
		...
		for (IConfigurationElement elmt : registry
				.getConfigurationElementsFor(perferencePage)) {
			...
			if (elmt.getAttribute(ATTR_CLASS) != null) {
				...
				try {
					...
					if (!(object instanceof PreferencePage)) {
						logger.error(MessageFormat.format(StringConstants.INL_LOG_ERROR_EXPECTED_INSTANCE_OF_PREF_PAGE, elmt.getAttribute(ATTR_CLASS)));
						continue;
					}
	...
	private void setPreferenceStore(String bundleId, PreferencePage page) {
		...
		if (store != null)
			...
			logger.warn(MessageFormat.format(StringConstants.INL_LOG_WARN_CANNOT_SET_PREF_STORE_FOR_PAGE, page.getTitle(), bundleId));
		}
	...
	private void initialisePreferenceStoreProviders() {
		if (psProviders == null) {
			...
			for (IConfigurationElement elmt : registry
				...
				if (isEmpty(pluginId)) {
					logger.warn(MessageFormat.format(StringConstants.INL_LOG_WARN_MISSING_PLUGIN_ID_IN_EXT, PREF_STORE_PROVIDER, declaringBundle));
					continue;
				}
				...
				if ((isEmpty(classname) && isEmpty(objectId))
						|| (((classname != null) && classname.length() > 0) && ((objectId != null) && objectId
								.length() > 0))) {
					logger.warn(MessageFormat.format(StringConstants.INL_LOG_WARN_IN_EXT_ONLY_1_OR_2_ATTR_MUST_BE_SET, PREF_STORE_PROVIDER, declaringBundle));
					continue;
				}
				...
				if (classname != null) {
					try {
						String prefStoreProviderURI = getClassURI(pluginId,
								elmt.getAttribute(ATTR_CLASS));

						data = factory.create(prefStoreProviderURI, context);
						if (!(data instanceof IPreferenceStoreProvider)) {
							logger.warn(MessageFormat.format(StringConstants.INL_LOG_WARN_IN_EXT_CLASS_MUST_IMPL_IPREFERENCESTOREPROVIDER, PREF_STORE_PROVIDER, declaringBundle));
							continue;
						}
